...
nrg <- readRDS("dataSets/states.rds")
head(nrg)
plot(nrg$metro, nrg$energy)


##   2. Print and interpret the model `summary'

enReg <- lm(energy ~ metro, data = nrg)
summary(enReg)

# Interpretation - Not only does the preliminary plot make this seem like there isn't much correlation between metro and
# energy, the linear model seems not great with a multiple R-squared of 0.1154. Interestingly, there does seem to be 
# some type of correlation as R gives it a * significance rating.
...
plot(nrg$pop, nrg$energy)

enReg2 <- lm(energy ~ metro + pop, data = nrg)
summary(enReg2)
plot(enReg2)

# This isn't much better so I will try something else. I also tried log(pop), which was better than without log, but 
# still no percievable meaningful difference.

#some plots of potential variables vs. energy
plot(nrg$waste, nrg$energy)
plot(nrg$area, nrg$energy)
plot(nrg$income, nrg$energy)
plot(nrg$green, nrg$energy)

# new model based on those plots
enReg3 <- lm(energy ~ metro + area + green, data = nrg)
summary(enReg3)
plot(enReg3)
# It seems with these, area and metro are now insignificant and green seems very signifacant, so one more modification
# to the model

enReg4 <- lm(energy ~ metro + green, data = nrg)
summary(enReg4)
plot(enReg4)
